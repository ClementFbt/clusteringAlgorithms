...
public class Compiere extends UDF implements Serializable
{
	/**
	 * 
	 */
	...
	/**
	 * 	Get Compiere Instance
	 *	@return Compiere Instance
	 */
	...
	/** Server Type						*/
	public static String 	s_type = TYPE_DB2;
	...
	*/
	 ...
	 *	@throws SQLException
	 */
	PreparedStatement prepareStatement (String sql, int resultSetType, int resultSetCurrency)
		throws SQLException
	{
		...
		try
		{
			if (s_conn != null)
			{
				...
				s_conn1 = null;  //jz no more need it here
				return ps;
			}
		}
		catch (Exception e)	//	connection not good anymore
		...
		synchronized (ONE)
		{
			//s_conn is bad at this time
			if (s_conn1 != null)
			{
				s_conn = s_conn1;
				s_conn1 = null;
			}
			else //try this at last
			try
			{
				s_conn = getConnection();
			}
	...
	public void setConnection(Connection conn, int sig)
	{
		synchronized (ONE)
		{
			if (s_conn != null)
			{
				if (s_conn == conn) //same reference, no need to set again
					return;
				else if (s_conn1 == null)
					s_conn1= conn; //as 2nd connection in the case the connection from others is bad
			}
			else
				s_conn = conn;
		}
	...
	//jz seems DB2 doesn't like static connection
	//private static Connection s_conn = null;
	//private static Connection s_conn1 = null;
	 ...
	 *	@param fmt String, 'D', 'M', 'Y', etc. ignored for now
	 *	@return day
	 */
	/* jz use the one from TimeUtil.java
	public static Timestamp trunc (Timestamp p_dateTime, String fmt)
	{
	...
	*/
	...
	/** Truncate Day - D			*/
	public static final String	TRUNC_DAY = "DD"; //jz user Oracle format 'DD', 'DY'
	/** Truncate Week - W			*/
	...
	static public Timestamp trunc (Timestamp dayTime, String trunc)
	{
		...
		//jz GregorianCalendar cal = new GregorianCalendar(Language.getLoginLanguage().getLocale());
		GregorianCalendar cal = new GregorianCalendar();
	...
	//jz to make work for both java in this package and function written in the code
	 ...
	 *	@throws SQLException
	 */
	public int nextID (int AD_Sequence_ID, String System)
	 ...
	 */
	public void ProcNextID (int AD_Sequence_ID, String System, int[] retVal)
	throws SQLException, Exception
	{
