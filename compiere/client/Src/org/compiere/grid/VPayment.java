	...
	implements ActionListener
{
	...
	public VPayment (int WindowNo, GridTab mTab, VButton button)
	{
		...
		try
		{
			bDateField = new VDate("DateAcct", false, false, true, DisplayTypeConstants.Date, "DateAcct");
			jbInit();
	...
	//		return false;
		...
		if (m_mTab.getValue("C_Payment_ID") != null)
		{
			...
			if (m_C_Payment_ID != 0)
			{
				...
				boolean canChange = !m_mPayment.isApproved() || !(m_PaymentRule==X_C_Order.PAYMENTRULE_CreditCard);
		...
		for (Object element : a) {
			String PaymentRule = (String)element;		//	used for Panel selection
			if (X_C_Order.PAYMENTRULE_DirectDebit.equals(PaymentRule)			//	SO
				...
				&& m_isSOTrx)
				continue;
			ValueNamePair pp = new ValueNamePair(PaymentRule, values.get(element));
			paymentCombo.addItem(pp);
			if (PaymentRule.equals(m_PaymentRule))	//	to select
				vp = pp;
		}
		...
		vp = null;
		for (ValueNamePair element : ccs) {
			kTypeCombo.addItem(element);
			if (element.getValue().equals(m_CCType))
				vp = element;
		}
		//	Set Selection
		if (vp != null)
	...
	//	log.config( "VPayment.checkMandatory");
		...
		{
			if (PaymentRule.equals(X_C_Order.PAYMENTRULE_DirectDeposit)) {
			} else if (PaymentRule.equals(X_C_Order.PAYMENTRULE_DirectDebit)) {
			} else if (PaymentRule.equals(X_C_Order.PAYMENTRULE_Check)) {
			}
	...
	private void processOnline()
	{
		...
		if (PaymentRule.equals(X_C_Order.PAYMENTRULE_CreditCard))
		{
			...
			m_mPayment.setCreditCard(X_C_Payment.TRXTYPE_Sales, CCType,
				kNumberField.getText(), "", kExpField.getText());
			m_mPayment.setAmount(m_C_Currency_ID, m_Amount);
			m_mPayment.setPaymentProcessor();
			m_mPayment.setC_BPartner_ID(m_C_BPartner_ID);
