...
public final class MRole extends X_AD_Role
{
	...
	public static MRole getDefault ()
	{
		if (s_defaultRole == null)
		{
			if (!Ini.isClient())
			{
				s_log.log(Level.WARNING, "Server trying to access Swing Role", new IllegalArgumentException("Access to Client Role"));
		...
		if (gwtServerID > 0)
		{
			...
			if (wr != null)
			{
				...
				if (role != null)
					...
					s_log.warning("MRole for ServerID=" + gwtServerID + " has already been garbage collected");
			}
			else
	 ...
	 *	@return role
	 */
	public static MRole get (Ctx ctx, int AD_Role_ID, int AD_User_ID, boolean reload)
	{
		//	Create Swing
		...
		//	General
		s_log.finer("AD_Role_ID=" + AD_Role_ID + ", AD_User_ID=" + AD_User_ID + ", reload=" + reload);
	...
	/**
	 * 	Reset Gwt Cache
	 * 	@param gwtServerID gwt server id
	 * 	@return counter
	 */
	...
	public static int reset(int AD_Role_ID)
	{
		...
		//
		if (s_defaultRole != null && s_defaultRole.getAD_Role_ID() == AD_Role_ID)
		{
			s_defaultRole = null;
			counter++;
		}
		...
		//
		int cleanup = 0;
		Iterator<Map.Entry<Integer, SoftReference<MRole>>> it2 = s_gwtRoles.entrySet().iterator();
		while (it2.hasNext())
		{
	...
	/** GWT User Role				*/
    private static ConcurrentHashMap<Integer, SoftReference<MRole>> s_gwtRoles
    	= new ConcurrentHashMap<Integer, SoftReference<MRole>>();
	/** General User Role Cache		*/
	private static final CCache<String,MRole> s_roles = new CCache<String,MRole>("AD_Role", 5);
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		if (newRecord && success)
		{
			...
			MUserRoles su = new MUserRoles(getCtx(), SUPERUSER_USER_ID, getAD_Role_ID(), get_Trx());
			su.save();
			//	Add Role to User
			if (getCreatedBy() != SUPERUSER_USER_ID)
			{
				MUserRoles ur = new MUserRoles(getCtx(), getCreatedBy(), getAD_Role_ID(), get_Trx());
				ur.save();
			}
			updateAccessRecords();
		}
	...
	public String updateAccessRecords ()
	{
		...
		//
		int winDel = DB.executeUpdate("DELETE FROM AD_Window_Access" + whereDel, get_Trx());
		int win = DB.executeUpdate(sqlWindow + roleAccessLevelWin, get_Trx());
		int procDel = DB.executeUpdate("DELETE FROM AD_Process_Access" + whereDel, get_Trx());
		int proc = DB.executeUpdate(sqlProcess + roleAccessLevel, get_Trx());
		int formDel = DB.executeUpdate("DELETE FROM AD_Form_Access" + whereDel, get_Trx());
		int form = DB.executeUpdate(sqlForm + roleAccessLevel, get_Trx());
		int wfDel = DB.executeUpdate("DELETE FROM AD_Workflow_Access" + whereDel, get_Trx());
		int wf = DB.executeUpdate(sqlWorkflow + roleAccessLevel, get_Trx());
	...
	private static CCachePerm<Integer,String>		m_tableAccessLevel = new CCachePerm<Integer, String>("TableAccess", 20, 0, "AD_Table") {
	...
	/**	Table Name				*/
	private static ConcurrentHashMap<String,Integer>		m_tableName = new ConcurrentHashMap<String, Integer>(20);
	...
	{
		if ((m_tableAccessLevel.size() > 0 || m_tableName.size() > 0) && !reload)
			return;
	...
	public boolean isCanExport (int AD_Table_ID)
	{
		...
		for (int i = 0; i < m_tableAccess.length; i++)
		{
			...
			if (m_tableAccess[i].isExclude())		//	Exclude
			{
				// bug 10018373: added if condition (similar to the if condition in isCanReport)
				if (m_tableAccess[i].getAD_Table_ID() == AD_Table_ID)
				{
	...
	public boolean isTableAccessLevel (int AD_Table_ID, boolean ro)
	{
		...
		String tableAccessLevel = m_tableAccessLevel.get(null, Integer.valueOf(AD_Table_ID));
		...
		if (userLevel.indexOf('S') != -1
			&& (tableAccessLevel.equals(X_AD_Table.ACCESSLEVEL_SystemOnly)
				|| tableAccessLevel.equals(X_AD_Table.ACCESSLEVEL_SystemPlusTenant)))
			return true;
		if (userLevel.indexOf('C') != -1
			&& (tableAccessLevel.equals(X_AD_Table.ACCESSLEVEL_TenantOnly)
				|| tableAccessLevel.equals(X_AD_Table.ACCESSLEVEL_TenantPlusOrganization)
				|| tableAccessLevel.equals(X_AD_Table.ACCESSLEVEL_SystemPlusTenant)))
			return true;
	...
	public Boolean getTaskAccess (int AD_Task_ID)
	{
		if (m_taskAccess == null)
		{
			...
			try
			{
				...
				ResultSet rs = pstmt.executeQuery();
				while (rs.next())
					m_taskAccess.put(Integer.valueOf(rs.getInt(1)), Boolean.valueOf("Y".equals(rs.getString(2))));
				rs.close();
				pstmt.close();
				pstmt = null;
			}
	...
	public boolean canView(Ctx ctx, String TableLevel)
	{
		...
		else if (X_AD_Table.ACCESSLEVEL_TenantPlusOrganization.equals(TableLevel)
	...
	public boolean isDisplayClient()
	{
		...
		return s == null
			|| DISPLAYCLIENTORG_AlwaysTenantOrganization.equals(s);
	}	//	isDisplayClient
