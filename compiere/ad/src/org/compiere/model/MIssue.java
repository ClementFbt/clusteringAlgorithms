...
{
	/** */
    private static final long serialVersionUID = -7782639601599390380L;
    ...
    public static MIssue create (Ctx ctx, String hexInput)
	{
		...
		{
			byte[] byteArray = SecureUtil.convertHexString(hexInput);	
	 ...
	 */
	public MIssue (Ctx ctx, int AD_Issue_ID, Trx trx)
	{
	 ...
	 *	@param trx p_trx
	 */
	public MIssue (Ctx ctx, ResultSet rs, Trx trx)
	{
		super (ctx, rs, trx);
	}	//	MIssue
	...
	public MIssue (LogRecord record)
	{
		...
		if (t != null)
		{
			...
			int count = 0;
			for (StackTraceElement element : tes) {
	 ...
	 *	@param hmIn hash map
	 */
	public MIssue (Ctx ctx, HashMap<String,String> hmIn)
	{
		super (ctx, 0, null);
		load(hmIn);
		setRecord_ID(0);
	 ...
	 * 	@throws Exception
	 */
	private void init(Ctx ctx) throws Exception
	{
		...
		setVersion(Compiere.DATE_VERSION);		//	Code
		setDatabaseInfo(DB.getDatabaseInfo());
		setOperatingSystemInfo(Compiere.getOSInfo());
		setJavaInfo(Compiere.getJavaInfo());
		setReleaseTag(Compiere.getImplementationVersion());
	...
	public String createAnswer()
	{
		...
		if (getA_Asset_ID() != 0)
			sb.append("Sign up for support at http://www.compiere.org to receive answers.");
		else
		{
	...
	public String report()
	{
		...
		if (getRecord_ID() == 1)	//	new
		{
			...
			try		//	deserializing in create
			{
				...
				oOut.flush();
				String hexString = SecureUtil.convertToHexString(bOut.toByteArray());
