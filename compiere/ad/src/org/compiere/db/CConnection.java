...
public class CConnection implements Serializable
{
	...
	public void setDBType (String type)
	{
		...
		if (isOracle())
		{
			if (dbPort != DB_Oracle.DEFAULT_PORT)
		...
		else
		{
			...
			if (isDB2())
			{
				if (dbPort != DB_DB2.DEFAULT_PORT)
					setDbPort (DB_DB2.DEFAULT_PORT);
			}
			else if (isPostgreSQL())
			{
				if (dbPort != DB_PostgreSQL.DEFAULT_PORT)
					setDbPort (DB_PostgreSQL.DEFAULT_PORT);
			}
			else if (isMSSQLServer())
			{
				if (dbPort != DB_SQLServer.DEFAULT_PORT)
					setDbPort (DB_SQLServer.DEFAULT_PORT);
			}
		}
	} 	//  setType
