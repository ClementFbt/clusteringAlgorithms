...
public class TimeUtil
{
	...
	/**
	 * 	Is a day (hour/min/sec/ms = 0)
	 * 	@param date the date
	 * 	@return true if it is a day only
	 */
	static public boolean isDay (Timestamp date)
	{
	...
	/** Truncate Week - DAY			*/
	public static final String	TRUNC_WEEK = "DAY";
	/** Truncate Month - MM			*/
	 ...
	 *  @return truncated day with 00:00 
	 */
	...
	/**
	 * 	Get truncated day/time and set it to LAST day of period
	 *  @param dayTime day
	 *  @param trunc how to truncate TRUNC_*
	 *  @return truncated day with 00:00
	 */
	static public Timestamp truncLast (Timestamp dayTime, String trunc)
	{
		if (dayTime == null)
			dayTime = new Timestamp(System.currentTimeMillis());
		GregorianCalendar cal = new GregorianCalendar(Language.getLoginLanguage().getLocale());
		...
		if (trunc.equals(TRUNC_WEEK))
		{
			cal.set(Calendar.DAY_OF_WEEK, cal.getFirstDayOfWeek());		//	Sun
			cal.add(Calendar.DAY_OF_YEAR, 6);		//	add 6 days
			return new Timestamp (cal.getTimeInMillis());
		}
		// MM
		cal.set(Calendar.DAY_OF_MONTH, 1);
