...
public class UITab extends UITabVO {
	...
	public Object[][] executeQuery(QueryVO queryVO,
		...
		try {
			results = QueryUtil.executeQuery(sql1,
					whereClause.params.toArray(), 0, MAX_QUERY_RESULT_SIZE + 1);
		...
		for (String token : tokens) {
			...
			for (String forbidden : FORBIDDEN) {
				if (forbidden.equals(token)) {
					throw new IllegalStateException("Can't use word:" + token
		...
		if (!newRecord && cachedRow != null) {
			...
			if (fields.size() > 0) {
				ChangeVO c = new ChangeVO(true,
						"Please refresh the row. The following fields are updated by others:"
						+ fields);
				return c;
			}
		}
	...
	private PO getPO(CContext ctx, int windowNo, boolean newRecord) {
		...
		if (newRecord) {
			PO po = table.getPO(ctx, 0, null); // always returns new
	...
	private String checkReadOnly(CContext ctx, int windowNo) {
		...
		if (!role.canUpdate(AD_Client_ID, AD_Org_ID, getAD_Table_ID(),
			...
			String errorMsg = "@UserCannotUpdate@";
	...
	private void processCalloutExternal(ChangeVO change, PO po, int windowNo,
		...
		try {
			...
			method.invoke(instance, po, field, oldValue, newValue);
		} catch (Exception e) {
