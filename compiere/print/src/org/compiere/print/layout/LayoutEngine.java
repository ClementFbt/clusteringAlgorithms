...
public class LayoutEngine implements Pageable, Printable, Doc
{
	...
	public void setPrintFormat (MPrintFormat format, boolean doLayout)
	{
		...
		m_printCtx = new Ctx(format.getCtx());

		//	Set Paper
		boolean tempHasLayout = m_hasLayout;
		m_hasLayout = false;	//	do not start re-calculation
		MPrintPaper mPaper = MPrintPaper.get(getCtx(), format.getAD_PrintPaper_ID());
		...
		m_printFont = MPrintFont.get (getCtx(), format.getAD_PrintFont_ID());

		//	Print Context
		m_printCtx.setContext(Page.CONTEXT_REPORTNAME, m_format.getName());
		m_printCtx.setContext(Page.CONTEXT_HEADER, Env.getHeader(m_printCtx, 0));
	...
	private void layout()
	{
		...
		m_printCtx.setContext(Page.CONTEXT_DATE,
			DisplayType.getDateFormat(DisplayTypeConstants.Date, m_format.getLanguage()).format(now));
		m_printCtx.setContext(Page.CONTEXT_TIME,
			DisplayType.getDateFormat(DisplayTypeConstants.DateTime, m_format.getLanguage()).format(now));
		//	Update Page Info
	...
	private void createStandardHeaderFooter()
	{
		...
		PrintElement element = new ImageElement(org.compiere.Compiere.getImageLogoSmall(true), "CompiereImageLogoSmall");	//	48x15
	//	element = new ImageElement(org.compiere.Compiere.getImageLogo());	//	100x30
		element.layout(48, 15, false, X_AD_PrintFormatItem.FIELDALIGNMENTTYPE_LeadingLeft);
		...
		element.layout (m_header.width, 0, true, X_AD_PrintFormatItem.FIELDALIGNMENTTYPE_Center);
		element.setLocation(m_header.getLocation());
		m_headerFooter.addElement(element);
		//
		//
		element = new StringElement("@Page@ @*Page@ @of@ @*PageCount@", font, color, null, true);
		element.layout (m_header.width, 0, true, X_AD_PrintFormatItem.FIELDALIGNMENTTYPE_TrailingRight);
		element.setLocation(m_header.getLocation());
		m_headerFooter.addElement(element);
	...
	//	element = new StringElement(Compiere.NAME, font, color, null, true);
		element.layout (m_footer.width, 0, true, X_AD_PrintFormatItem.FIELDALIGNMENTTYPE_LeadingLeft);
		...
		element.layout (m_footer.width, 0, true, X_AD_PrintFormatItem.FIELDALIGNMENTTYPE_Center);
		element.setLocation(ft);
		m_headerFooter.addElement(element);
		//
		element = new StringElement("@*CurrentDateTime@", font, color, null, true);
		element.layout (m_footer.width, 0, true, X_AD_PrintFormatItem.FIELDALIGNMENTTYPE_TrailingRight);
		element.setLocation(ft);
		m_headerFooter.addElement(element);
	}	//	createStandardHeaderFooter
		...
		for (int row = 0; row < m_data.getRowCount(); row++)
		{
			...
			//	log.fine("layoutForm - Row=" + row + " - #" + i + " - " + item);
				...
				if (item.isRelativePosition())
				{
					...
					{
						alignment = X_AD_PrintFormatItem.FIELDALIGNMENTTYPE_LeadingLeft;
						maxWidth = getAreaBounds().width;
						lineAligned = true;
					}
					else if (item.isLineAlignCenter())
					{
						alignment = X_AD_PrintFormatItem.FIELDALIGNMENTTYPE_Center;
						maxWidth = getAreaBounds().width;
						lineAligned = true;
					}
					else if (item.isLineAlignTrailing())
					{
						alignment = X_AD_PrintFormatItem.FIELDALIGNMENTTYPE_TrailingRight;
						maxWidth = getAreaBounds().width;
						lineAligned = true;
					}
				}
	...
	private PrintElement includeFormat (MPrintFormatItem item, PrintData data)
	{
		...
		query.addRestriction(item.getColumnName(), Query.EQUAL, new Integer(Record_ID));
		...
		int maxWidth, int maxHeight, boolean isHeightOneLine, String FieldAlignmentType, boolean isTranslated)
	{
		...
		{
			MPrintFont f = MPrintFont.get (getCtx(), AD_PrintFont_ID);
			if (f.getFont() != null)
				font = f.getFont();
		}
		...
		String FieldAlignmentType, boolean isForm)
	{
		...
		if (data.isNull() && item.isSuppressNull())
			return null;
		String stringContent = data.getValueDisplay (m_format.getLanguage(), -1);
		...
		{
			MPrintFont f = MPrintFont.get (getCtx(), item.getAD_PrintFont_ID());
			if (f.getFont() != null)
				font = f.getFont();
		}
		...
		PrintElement e = null;
		if (data.getDisplayType() == DisplayTypeConstants.Location)
		{
	...
	private PrintElement createImageElement (MPrintFormatItem item)
	{
		...
		if (data.isNull() && item.isSuppressNull())
			return null;
		String url = data.getValueDisplay (m_format.getLanguage(), -1);
		...
		int xOffset)
	{
		...
		MPrintFont printFont = MPrintFont.get (getCtx(), format.getAD_PrintFont_ID());
		...
		for (int c = 0; c < format.getItemCount(); c++)
		{
			...
			if (item.isPrinted())
			{
				...
				if (columnJustification[col] == null || columnJustification[col].equals(X_AD_PrintFormatItem.FIELDALIGNMENTTYPE_Default))
					columnJustification[col] = X_AD_PrintFormatItem.FIELDALIGNMENTTYPE_LeadingLeft;	//	when generated sets correct alignment
				//	Column Fonts
				if (item.getAD_PrintFont_ID() != 0 && item.getAD_PrintFont_ID() != format.getAD_PrintFont_ID())
				{
					MPrintFont font = MPrintFont.get(getCtx(), item.getAD_PrintFont_ID());
	...
	//	System.out.println("Rows=" + rows);
					...
					else
					{
						...
						else if (obj instanceof PrintDataElement)
						{
							...
							else
							{
								...
								for (int ci = 0; ci < printData.getColumnInfo().length; ci++)
								{
									pdc = printData.getColumnInfo()[ci];
									if ( pde.getColumnName().equals(pdc.getColumnName()) )
									{
										int minFractionDigits = pdc.getMinFractionDigits();
										dataElement = pde.getValueDisplay(format.getLanguage(), minFractionDigits);
									}
								}
							}
						}
		...
		TableElement table = new TableElement(columnHeader,
			...
			tf, pageBreak);
		table.layout(0,0,false, X_AD_PrintFormatItem.FIELDALIGNMENTTYPE_LeadingLeft);
	...
	{
		if (m_query == null || !m_query.isActive() || m_query.isReportParameter())
			return null;
		//
